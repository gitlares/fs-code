...
actor ResponsesStreamState {
    ...
    private var completedOutputItems: [Int: ResponsesOutputItem] = [:]
    ...
    func recordCompletedOutputItem(
        _ item: ResponsesOutputItem,
        at outputIndex: Int,
        diagnostics: StreamFailureDiagnostics
    ) throws {
        guard outputIndex >= 0, completedOutputItems[outputIndex] == nil else {
            throw finalizedSemanticStateDiverged(diagnostics: diagnostics, field: "output-completions")
        }
        completedOutputItems[outputIndex] = item
    }

    func completedOutput(
        whenTerminalOutputIsEmpty terminalOutputIsEmpty: Bool,
        diagnostics: StreamFailureDiagnostics
    ) throws -> [ResponsesOutputItem]? {
        guard terminalOutputIsEmpty else { return nil }
        guard !completedOutputItems.isEmpty else { return nil }

        let indices = completedOutputItems.keys.sorted()
        guard indices == Array(0..<indices.count) else {
            throw finalizedSemanticStateDiverged(diagnostics: diagnostics, field: "output-indices")
        }
        return indices.compactMap { completedOutputItems[$0] }
    }

    func reconciliationDeltas(
        response: ResponsesAPIResponse,
        projection: ResponsesAPIClient.ResponsesTurnProjection,
        completedOutput: [ResponsesOutputItem]?,
        diagnostics: StreamFailureDiagnostics
    ) throws -> [StreamDelta] {
        let target = try ResponsesCompletedSemanticTarget(
            response: response,
            projection: projection,
            output: completedOutput ?? response.output
        )
        var deltas: [StreamDelta] = []

        guard let reasoningSuffix = utf8Suffix(of: target.reasoning, afterPrefix: reasoning) else {
            throw finalizedSemanticStateDiverged(diagnostics: diagnostics, field: "reasoning")
        }
        ...
        guard target.reasoningDetails.count >= reasoningDetails.count,
            ...
            throw finalizedSemanticStateDiverged(diagnostics: diagnostics, field: "reasoning-details")
        }
        ...
        guard let contentSuffix = utf8Suffix(of: target.content, afterPrefix: content) else {
            throw finalizedSemanticStateDiverged(
                diagnostics: diagnostics,
                field: target.content.isEmpty ? "content-empty" : "content"
            )
        }
        ...
        guard Set(toolCalls.keys).isSubset(of: targetIndices) else {
            throw finalizedSemanticStateDiverged(diagnostics: diagnostics, field: "tool-indices")
        }
        ...
        for (index, targetCall) in target.toolCalls.sorted(by: { $0.key < $1.key }) {
            if let existing = toolCalls[index] {
                guard existing.id == nil || existing.id == targetCall.id,
                    ...
                    throw finalizedSemanticStateDiverged(diagnostics: diagnostics, field: "tool-arguments")
                }
    ...
    private func finalizedSemanticStateDiverged(diagnostics: StreamFailureDiagnostics, field: String) -> AgentError {
        AgentError.llmError(.streamFailed(.malformedStream(
            reason: .finalizedFieldDiverged(field: field),
            diagnostics: diagnostics
        )))
    }
}
