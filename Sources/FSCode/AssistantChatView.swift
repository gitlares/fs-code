...
final class AssistantChatView: NSView, NSTableViewDataSource, NSTableViewDelegate, NSTextViewDelegate {
    ...
    override func viewDidMoveToWindow() {
        ...
        observerID = manager.addObserver { [weak self] in
            // The draft (and therefore the composer's cleared/restored state) must track
            // the manager immediately: scheduleRefresh() debounces the full interface
            // rebuild by 80ms, which is long enough for a just-sent message to already be
            // visible in the transcript while the composer still shows the old draft.
    ...
    private func buildInterface() {
        ...
        configureIconButton(newChatButton, symbol: "plus", label: "New chat", target: self, action: #selector(createChat))
        configureIconButton(
        ...
        configureIconButton(sendButton, symbol: "arrow.up.circle.fill", label: "Send message", target: self, action: #selector(sendMessage), bezelStyle: .rounded)
        configureIconButton(stopButton, symbol: "stop.circle.fill", label: "Stop response", target: self, action: #selector(stopResponse))
        ...
        composerContainer.layer?.cornerRadius = Radius.medium
    ...
    @objc private func sendMessage() {
        ...
        Task { [weak self] in
            ...
            // The manager accepts and clears the durable draft asynchronously. Refresh the
            // text view at that boundary as well as through observers so stale AppKit text
            // storage cannot leave an already-sent message visible in the composer.
            self.updateComposerState()
